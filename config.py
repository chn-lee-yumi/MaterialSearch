...
# *****扫描配置*****
ASSETS_PATH = tuple(os.getenv('ASSETS_PATH', '/home').split(','))  # 素材所在的目录，绝对路径
SKIP_PATH = tuple(os.getenv('SKIP_PATH', '/tmp').split(','))  # 跳过扫描的目录，绝对路径
IMAGE_EXTENSIONS = tuple(os.getenv('IMAGE_EXTENSIONS', '.jpg,.jpeg,.png,.gif').split(','))  # 支持的图片拓展名
VIDEO_EXTENSIONS = tuple(os.getenv('VIDEO_EXTENSIONS', '.mp4,.flv,.mov,.mkv').split(','))  # 支持的视频拓展名
IGNORE_STRINGS = tuple(os.getenv('IGNORE_STRINGS', 'thumb,avatar,__MACOSX,icons,cache').split(','))  # 如果路径或文件名包含这些字符串，就跳过（不区分大小写）
...
AUTO_SCAN_START_TIME = tuple(map(int, os.getenv('AUTO_SCAN_START_TIME', '22,30').split(',')))  # 自动扫描开始时间(时, 分)
AUTO_SCAN_END_TIME = tuple(map(int, os.getenv('AUTO_SCAN_END_TIME', '8,0').split(',')))  # 自动扫描结束时间(时, 分)
...
LANGUAGE = os.getenv('LANGUAGE', 'Chinese')  # 模型搜索时用的语言，可选：Chinese/English
MODEL_NAME = os.getenv('MODEL_NAME', 'openai/clip-vit-base-patch32')  # CLIP模型
TEXT_MODEL_NAME = os.getenv('TEXT_MODEL_NAME', 'IDEA-CCNL/Taiyi-CLIP-Roberta-102M-Chinese')  # 中文模型，需要和CLIP模型配套使用，如果LANGUAGE为English则忽略此项
DEVICE = os.getenv('DEVICE', 'cpu')  # 推理设备，cpu/cuda/mps，建议先跑benchmark.py看看cpu还是显卡速度更快。因为数据搬运也需要时间，所以不一定是GPU更快。
DEVICE_TEXT = os.getenv('DEVICE_TEXT', 'cpu')  # text_encoder使用的设备，如果LANGUAGE为English则忽略此项。
