...
ASSETS_PATH = tuple(os.getenv('ASSETS_PATH', 'G:/mi10ultra/DCIM/Camera.new,/srv').split(','))  # 素材所在的目录，绝对路径，逗号分隔 C:/Users/rui/Desktop/baiduapi/ImageSimilarity-main/9yuezhihou/testImageSimiHeic
...
VIDEO_EXTENSIONS = tuple(os.getenv('VIDEO_EXTENSIONS', '.flv,.mov,.mkv,.webm,.avi').split(','))  # 支持的视频拓展名，逗号分隔，请填小写 .mp4,
...
SCAN_PROCESS_BATCH_SIZE = int(os.getenv('SCAN_PROCESS_BATCH_SIZE', 64))  # 等读取的帧数到这个数量后再一次性输入到模型中进行批量计算，从而提高效率。显存较大可以调高这个值。
AUTOPROCESSOR_BATCH_SIZE = int(os.getenv('AUTOPROCESSOR_BATCH_SIZE', 4))  # 影响内存占用 AUTOPROCESSOR_BATCH_SIZE=4 num_workers=4 约增加0.5GB*4内存
NUM_WORKERS = int(os.getenv('NUM_WORKERS', 4)) # 4进程并行，每个进程固定占用2.7GB内存
...
# 8G显存推荐参数：小模型，SCAN_PROCESS_BATCH_SIZE=32 无论使用cpu或cuda，均会占用内存大约8G
